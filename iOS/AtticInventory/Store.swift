...
final class Store: ObservableObject {
    ...
    private let fileURL: URL = {
        ...
        return dir.appendingPathComponent("atticinventory_entries.json")
    }()
    ...
    static func seedData() -> [ItemEntry] {
        ...
        let now = Date()
        return [
            ItemEntry(date: cal.date(byAdding: .day, value: -30, to: now) ?? now, binLocation: "Sample entry 1", notes: "Getting started"),
            ItemEntry(date: cal.date(byAdding: .day, value: -10, to: now) ?? now, binLocation: "Sample entry 2", notes: "")
        ]
    }
}
